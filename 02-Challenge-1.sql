...
-----------------
SELECT * FROM animals 
where class IN ('mammal', 'reptile') AND monthly_visitors > 10000;

-- Determine the total maintance cost
SELECT SUM(maintenance_cost) AS total_cost FROM animals
...
SELECT AVG(weight)::INT FROM animals
WHERE gender = true;
...
-- Find the name and type of the most visited animalS
SELECT NAME, TYPE FROM ANIMALS
WHERE monthly_visitors = (
SELECT  MAX(monthly_visitors) FROM ANIMALS
);
